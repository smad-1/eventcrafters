    ...
    for (let i = 0; i < activityList.length; i++) {
      ...
      if (activityList[i] != undefined || activityList[i] != 'null') {
        ...
        if (activityList[i] != "null") {
          conn.query('INSERT INTO rentlots (userID, lotID, First_name, Last_name, Email, Phone_no, Lot_Name, Capacity, events, Area, Address, Extra_info) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [user.curr_user, savelotID.lotid, fName, lName, email, phone_no, lotname, capacity, activityList[i], area, addr, info], function (error, results, fields) {
              if (error) {
